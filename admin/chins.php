...
<body>
  ...
  if (isset($_POST['save'])) {
    ...
    $sql = "UPDATE `chinese_items` SET `file_path` = '$path', `name` = '$name', `price` = '$price', `aviability` = '$avail' WHERE `chinese_items`.`id` = $id;";
    $result = mysqli_query($conn, $sql);
  }
  if (isset($_POST['remove'])) {
    $id = $_POST['id'];

    $q1 = "DELETE FROM `chinese_items` WHERE `chinese_items`.`id` = $id";
