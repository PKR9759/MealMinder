...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['save'])) {
        ...
        $sql = "UPDATE `chinese_items` SET `file_path` = '$path', `name` = '$name', `price` = '$price', `aviability` = '$avail' WHERE `id` = $id";
        $result = mysqli_query($conn, $sql);
    }

    if (isset($_POST['remove'])) {
        $id = $_POST['id'];
        $q1 = "DELETE FROM `chinese_items` WHERE `id` = $id";
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <script>
            function openModal(itemId) {
                fetch(`/food2/admin/fetch_items.php?id=${itemId}&table=chinese_items`)
