...
<body>
  ...
  if (isset($_POST['save'])) {
    ...
    $sql = "UPDATE `snacks` SET `file_path` = '$path', `name` = '$name', `price` = '$price', `aviability` = '$avail' WHERE `snacks`.`id` = $id;";
    $result = mysqli_query($conn, $sql);
  }
  if (isset($_POST['remove'])) {
    $id = $_POST['id'];

    $q1 = "DELETE FROM `snacks` WHERE `snacks`.`id` = $id";
  ...
  if ($num) {
    ...
    while ($row = mysqli_fetch_assoc($result)){
      ...
      echo '<input type="hidden" name="cat" value="' . "snacks" . '" />';
