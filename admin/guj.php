...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['save'])) {
        ...
        $sql = "UPDATE `gujarati_items` SET `file_path` = '$path', `name` = '$name', `price` = '$price', `aviability` = '$avail' WHERE `id` = $id";
        $result = mysqli_query($conn, $sql);
    }

    if (isset($_POST['remove'])) {
        $id = $_POST['id'];
        $q1 = "DELETE FROM `gujarati_items` WHERE `id` = $id";
    ...
    echo '</form>';
        ...
        <script>
            function openModal(itemId) {
                fetch(`/food2/admin/fetch_items.php?id=${itemId}&table=gujarati_items`)
