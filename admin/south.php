...
<body>
  ...
  if (isset($_POST['save'])) {
    ...
    $sql = "UPDATE `south_items` SET `file_path` = '$path', `name` = '$name', `price` = '$price', `aviability` = '$avail' WHERE `south_items`.`id` = $id;";
    $result = mysqli_query($conn, $sql);
  }
  if (isset($_POST['remove'])) {
    $id = $_POST['id'];

    $q1 = "DELETE FROM `south_items` WHERE `south_items`.`id` = $id";
